...
<textarea name="descripcion" class="w-full px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 focus:outline-none focus:border-green-500">{{ $producto->descripcion }}</textarea>
</div>
...
<select name="id_autor" value="{{ $producto->id_autor }}" required
class="w-full px-4 py-2 rounded-lg bg-slate-700 border border-slate-600 focus:outline-none focus:border-green-500">
...
<option value="{{ $fila->id }}" {{ $producto->id_categoria == $fila->id ? 'selected' : '' }}>
{{ $fila->nombre }}
</option>
@endforeach
</select>
...
<option value="{{ $fila->id }}" {{ $producto->id_serie == $fila->id ? 'selected' : '' }}>
{{ $fila->nombre }}
</option>
@endforeach
</select>
...
<option value="{{ $fila->id }}" {{ $producto->id_editorial == $fila->id ? 'selected' : '' }}>
{{ $fila->nombre }}
</option>
@endforeach
</select>
...
<input type="file" name="imagen" value="{{ $producto->imagen }}" required
